...
Reads stdin; emits one token per line to stdout on success (exit 0);
exits 2 on any parser error, printing one diagnostic line to stderr.
...
def main():
    ...
    # Success path: emit one token per line to stdout.
    out = sys.stdout
    for tok in tokens:
        out.write(tok)
        out.write("\n")
