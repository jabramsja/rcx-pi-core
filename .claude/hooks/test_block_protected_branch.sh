...
# Stdout must be exactly two NUL-terminated tokens ('git', 'status'),
# exit status 0. We measure 5 runs and use the median to guard against
# one-off scheduling noise.
#
# Bash `$(...)` command substitution STRIPS NUL bytes from its captured
# output, so we cannot capture NUL-delimited helper stdout into a bash
# variable for comparison. Instead we pipe the helper's stdout into a
# python3 reader that splits on NUL and validates the token list
# structurally. The python reader returns the literal string 'OK' on
# success, or a diagnostic on mismatch, which bash CAN capture.
probe_helper_overhead() {
  local name="helper-overhead"
  local check
  check=$(printf '%s' "git status" | python3 "$HELPER" | python3 -c '
import sys
data = sys.stdin.buffer.read()
tokens = data.split(b"\0")
# Trailing NUL (from the final token terminator) produces an empty
# final split entry - strip it.
if tokens and tokens[-1] == b"":
    tokens = tokens[:-1]
if tokens == [b"git", b"status"]:
    print("OK")
else:
    print("FAIL: got", tokens)
' 2>&1)
  if [ "$check" != "OK" ]; then
    FAIL=$((FAIL + 1))
    FAILURES+="
  [$name] stdout mismatch
    expected: 2 NUL-terminated tokens (b'git', b'status')
    actual:   $check"
    return
  fi
  local real_ms
