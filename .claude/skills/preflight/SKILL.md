...
14. Set up 5-minute identity + override refresh cron — create a recurring cron job that runs every 5 minutes: re-read MEMORY.md, check pipeline state via MCP SQLite, confirm overrides active, and self-audit against engineering identity with 6 questions: (1) What did you claim without verifying? (Would the review panel catch you?) (2) What code did you not read before modifying? (Malpractice check) (3) What error did you conjecture about instead of diagnosing? (Shortcut check) (4) What assumption did you not check? (Hypothesis-not-conclusion check) (5) Did you say "this works" without running the actual flow? (Proof check) (6) Did you skip or reorder any protocol steps? (Protocol fidelity check). If a cron is already running, skip. This is MANDATORY per `feedback_contradiction_detection.md`.
