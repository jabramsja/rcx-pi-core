...
- JS DEBT SUMMARY in `constants.js` lists 6 host operations (2 iteration + 2 recursion + 2 builtin). Canonical counts in `tools/checks/host_semantics_baseline.json` (20 total: 6 Py decorator + 6 JS decorator + 8 AST_OK bootstrap). See `mu/docs/core/Why_RCX_PI_VM_EXISTS.md` for why every host operation is tracked debt.
...
|--------|-------|------------------|-----------------|
| **Tracked markers** | 20 | Narrow official `@host_*` debt marker sites (6 Py decorator + 6 JS decorator + 8 AST_OK bootstrap). The semantic debt the project explicitly categorizes (host_builtin, host_iteration, host_mutation, host_recursion, AST_OK bootstrap). | `tools/checks/host_semantics_baseline.json` |
...
The 20 represents the current tracked marker count (6 Py decorator + 6 JS decorator + 8 AST_OK bootstrap — lower bound on total host work; L2 kernel + utilities + Stage0 VM). All 8 AST_OK bootstrap now included in threshold (previously 4 were excluded as "pre-dating threshold tracking"). P7-d adds _step_kernel_with_vm @host_iteration on Py (VM dispatch for match.v2/subst.v2 in step_kernel_mu). L4 paths are documented:
