...
**L4 Status:** G8 UNPROVEN (4/4 executable evidence, 2026-03-02). All four primitives have executable reducibility evidence. `eval_step` reclassified IRREDUCIBLE → REDUCIBLE_WITH staged bootstrap (D001 analytical + D002-D003 + D005 executable). `max_steps` confirmed REDUCIBLE_WITH CPS fuel threading (D006, 20 tests). `stack_guard` confirmed REDUCIBLE_WITH depth parameter (D009: research-only Python analogs; depth budget as Mu linked-list, 36 tests; production stack_guard unchanged; memoization/cycle-detection parity deferred). `projection_loader` confirmed REDUCIBLE_WITH binary format (D010: ~100 LOC custom recursive TLV decoder, 59 tests including golden byte fixtures and engine-level behavioral parity; research-only, production projection_loader unchanged; I/O, integrity, validation out of scope). Gate remains UNPROVEN pending formal adjudication. See `mu/docs/core/L4ExitChecklist.v0.md` for gate criteria and `mu/docs/core/L4DecisionCard.v0.md` for D001-D010 decision log.

**Post-D008 Operating Mode:** D008 GO rendered (founder, 2026-03-01; supersedes prior DEFER). D005 production pilot COMPLETE (PR #452 merged, 2026-03-01). Stage 0 micro-kernel integrated with pilot flag (default OFF), 90 gate tests + 9 contract tests. G8 evidence now 4/4 (2026-03-02): `eval_step`, `max_steps`, `stack_guard`, and `projection_loader` all have executable evidence. D010 (projection_loader binary format) EXECUTED: ~100 LOC custom recursive TLV decoder, 59 tests (17 golden byte fixtures, 2 canonical seed round-trips, 2 engine-level behavioral parity, 12 failure modes), H5 partially confirmed — JSON parsing is replaceable, I/O/integrity/validation remain host; research-only, production projection_loader unchanged. D009 (stack_guard depth threading) EXECUTED: research-only Python analogs, 36 tests, H4 partially confirmed — depth data is Mu, mechanism remains host; production stack_guard unchanged; memoization/cycle-detection parity and JS cross-substrate analog deferred as non-goals. Hemisphere Metabolization Contract COMPLETE (E1-E5 all MET, 2026-02-20). Boot1 shadow-merge COMPLETE (2026-02-19). All prior NEXT contracts closed. Wave 25 JS perf fix merged (PR #453, 42x speedup + non-linear hash fix + policy lock). P4 hotspot measured and DEFERRED (2026-03-02, PR #458).
