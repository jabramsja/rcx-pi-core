...
| **Authority sites** | 219 | Named runtime sites currently flagged by the broader authority inventory ratchet. Functions with host-authority signals (isinstance, loops, builtins, recursion) across the runtime tree. Per-substrate: 123 Python + 96 JavaScript. | `tools/checks/host_authority_inventory_baseline.json` (authority inventory) |
| **Total inventory sites** | 309 | Full named host-runtime surface in scope. Every function in the runtime tree that touches any host-language construct. Per-substrate: 180 Python + 129 JavaScript. | `tools/checks/host_authority_inventory_baseline.json` (total inventory) |

**Why three ledgers:** The 16 tracked markers are the narrow debt the project has categorized and accepted. The 219 authority sites are the broader surface the ratchet prevents from growing. The 309 total inventory sites are the full host-runtime footprint — the upper bound on what "self-hosting" must eventually eliminate or accept as irreducible bootstrap.
...
The 16 represents the current tracked marker count (6 Py decorator + 6 JS decorator + 4 AST_OK bootstrap — lower bound on total host work; L2 kernel + utilities + Stage0 VM). All 8 AST_OK bootstrap now included in threshold (previously 4 were excluded as "pre-dating threshold tracking"). P7-d adds _step_kernel_with_vm @host_iteration on Py (VM dispatch for match.v2/subst.v2 in step_kernel_mu). L4 paths are documented:
...
**Last updated:** 2026-03-17 (Wave 3C match VM staged dispatch, authority 219, total 309, factory loader consolidation)
