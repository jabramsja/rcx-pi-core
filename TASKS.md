...
- Tracker sync note (2026-02-22, wave11-test-path-hardening): **Fix gate test REPO_ROOT resolution for mu/tests/ invocation — MAINTENANCE.** Class: MAINTENANCE. target_gate_id: G8. NO_OP_PROOF: test-only — fixes fragile `Path(__file__).parents[2]` root resolution in `test_js_observer_api_guard_gate.py` that failed when invoked via `mu/tests/` (resolved to `<repo>/mu` instead of `<repo>`). Replaced with `from tests.repo_root import REPO_ROOT` (shared upward-search helper). defer_reason_code: TEST_PATH_ROBUSTNESS — gate test must work from both `tests/` symlink and `mu/tests/` canonical paths. evidence_command: `pytest tests/l4_gates/test_js_observer_api_guard_gate.py -q && pytest mu/tests/l4_gates/test_js_observer_api_guard_gate.py -q`. No runtime change. No phase/debt change.
