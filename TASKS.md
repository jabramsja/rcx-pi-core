...
- Tracker sync note (2026-02-22, l4-maint-wave11b-gate-root-hardening): **Harden all L4 gate test REPO_ROOT resolution for mu/tests/ invocation — MAINTENANCE.** Class: MAINTENANCE. target_gate_id: G8. NO_OP_PROOF: test-only — replaces fragile `Path(__file__).parents[2]` with shared `tests.repo_root.REPO_ROOT` upward-search helper in all 9 L4 gate test files. defer_reason_code: TEST_PATH_ROBUSTNESS — gate tests must work from both `tests/` symlink and `mu/tests/` canonical paths. evidence_command: `pytest -q tests/l4_gates/ && pytest -q mu/tests/l4_gates/`. No runtime change. No phase/debt change.
