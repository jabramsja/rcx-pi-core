...
- Tracker sync note (2026-02-22, l4-structural-wave13-python-error-code-parity): **Typed Python error_code parity for engine fail-closed outcomes — L4_STRUCTURAL.** Class: L4_STRUCTURAL. target_gate_id: G8. evidence_command: `pytest tests/l4_gates/test_python_error_code_parity_gate.py -q`. host_semantics_delta_before: Python engine error paths raised bare `RuntimeError` with message text only. Tests asserted via `pytest.raises(RuntimeError, match="exhausted")` — message-pattern matching. No machine-readable error code attribute. JS had `RcxError` with `.error_code` since Wave 1; parity tests required different assertion patterns per substrate. host_semantics_delta_after: Python engine error paths now raise `RcxEngineError(RuntimeError)` with `.error_code` attribute mirroring JS `RcxError`. 8 raise sites converted: 2x `engine.exhausted`, 2x `engine.stalled_non_terminal`, 1x `engine.boot1_depth_exceeded`, 3x `input.shape_mismatch`. Subclasses `RuntimeError` so all existing `except RuntimeError` and `pytest.raises(RuntimeError, match=...)` patterns remain valid. structural_artifact_ref: mu/host/python/rcx_pi/selfhost/step_mu.py (RcxEngineError class + 8 raise sites in run_engine_pipeline, _run_engine_recursive, run_hemisphere_routing, run_engine_with_routing). evidence_delta: 14 gate tests: 3 exception class contract (subclass, attribute, catch), 4 Python error-code presence + message preservation (exhausted + shape), 2 backward compatibility (legacy RuntimeError catch patterns), 2 cross-substrate parity (engine.exhausted + input.shape_mismatch), 3 source lock (all 4 codes in both substrates + count). No JS change. No phase/debt change.
