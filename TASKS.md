...
- Tracker sync note (2026-04-11, block-protected-branch-lexer-2026-04-11): **fix: replace sed-regex with Python state-machine tokenizer in block-protected-branch.sh — permanently eliminates recursive sed-regex P1 regression pattern.**. Class: L4_ENABLER. target_gate_id: G8. evidence_command: `bash .claude/hooks/test_block_protected_branch.sh && bash tools/pre-push-fast`. evidence_delta: (1) 16-scenario smoke test suite PASS (9 plan scenarios A-K + 5 edge cases K1-K5 + helper-level direct probes). (2) Python state-machine tokenizer with explicit states (UNQUOTED/SINGLE_QUOTED/DOUBLE_QUOTED/ESCAPE_UNQUOTED/ESCAPE_DOUBLE/COMMENT) + at_word_boundary tracking + fail-closed ValueError contract (rc=2 on malformed input). (3) Helper overhead median 61ms, well under the 200ms acceptance bound. (4) Pre-push-fast PASSED end-to-end in the worktree with all 4 wave files. (5) Phase A bridge converged in 2 rounds (R1 REQUEST_CHANGES 3 blocking → Rewrite 1 → R2 GO). progress_proof_before: block-protected-branch.sh used a v3 sed-regex `(^|[[:space:]])#.*$` at line 54 of commit 24e630c that incorrectly treated whitespace+hash inside quoted strings as comment starts (bot reproduced `echo ' #foo'; git commit -m x` bypassing the protected-branch guard on main branch — PR #754 post-merge P1 bot finding 2026-04-11T18:18:31Z, unaddressed safety regression live on dev). progress_proof_after: new Python state-machine tokenizer handles all quote/comment/escape cases correctly, including the PR #754 quoted-space-hash case plus 15 other scenarios. Helper exits fail-closed on malformed input via ValueError → rc=2 → hook emits BLOCK decision with attributable reason. Sed-regex class of bugs eliminated at the lexical level. primary_blocker_class: INTEGRATION. primary_invariant_id: INV_STRUCTURAL_FORWARD_MOTION. indicator_artifact_ref: reports/l4_wave_indicators/block-protected-branch-lexer-2026-04-11.json. indicator_collection_command: python3 mu/tools/metrics/collect_l4_wave_indicators.py --wave-id block-protected-branch-lexer-2026-04-11 --output reports/l4_wave_indicators/block-protected-branch-lexer-2026-04-11.json. bootstrap_endgame_policy: SUBSTRATE_INDEPENDENT_MINIMAL_BOOTSTRAP. boot0_track_id: V1. boot0_progress_state: HOLD. FOUNDER_OVERRIDE:block-protected-branch-lexer-2026-04-11 (founder directed structural tokenizer rewrite to break the recursive sed-regex P1 cycle — 3 consecutive PR bot findings on PR #746 and PR #754 proved regex cannot correctly parse bash comment/quote semantics; non-structural adjacency bypass required — this is the 7th consecutive pipeline-hardening wave in the chain after #751/#752/#753/#754 and the in-flight bot-findings follow-up). Sub-wave of [ANTI-DRIFT-ENFORCEMENT] parent task authority at TASKS.md:152-155.
