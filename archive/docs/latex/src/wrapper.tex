...
% --- RCX CANON: engine-aware Unicode font setup ---
\usepackage{iftex}
\ifPDFTeX
  % pdfLaTeX fallback path
  \usepackage[T1]{fontenc}
  \usepackage[utf8]{inputenc}
  \usepackage{lmodern}
\else
  % XeLaTeX/LuaLaTeX path (Unicode-safe, full glyph coverage)
  \usepackage{fontspec}
  \defaultfontfeatures{Ligatures=TeX}

  % Use macOS system fonts (available on default installs)
  \setmainfont{Times New Roman}
  \setsansfont{Helvetica}

  % Mono font for code blocks (needs π/μ/ω + box drawing + arrows + (check)(cross))
...
% --- RCX CANON: checkmark/crossmark glyph routing (avoids missing glyphs in body font) ---
% We do NOT use Unicode (check)/(cross) at all. Use dingbats via pifont (works in pdfLaTeX + XeLaTeX).
\usepackage{pifont}
\newcommand{\RCXcheck}{\ding{51}} % check mark
\newcommand{\RCXcross}{\ding{55}} % cross mark
% --- /RCX CANON: checkmark/crossmark glyph routing ---
...
s% --- RCX CANON: chapter includes (order is canonical) ---
\input{00-overview.tex}
\input{01-getting-started.tex}
\input{02-core-structures.tex}
\input{03-evaluator.tex}
\input{04-classification.tex}
\input{05-pretty-printing.tex}
\input{06-programs.tex}
\input{07-higher-operations.tex}
\input{08-self-hosting.tex}
\input{09-emergence.tex}
