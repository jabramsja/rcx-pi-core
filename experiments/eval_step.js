 ...
 *
 * Security hardened (v5 - L3 EngineNews Parity):
...
// =============================================================================
// Test 10: EngineNews Closure Detection (L3 Parity)
// =============================================================================

console.log('\n=== Test 10: EngineNews Closure Detection (L3 Parity) ===\n');

/**
 * Run EngineNews closure detection on a trace result.
 *
 ...
 */
function runEngineNews(traceResult) {
  // Wrap in _detect_closure format for EngineNews projections
  const enginenewsInput = {
    _detect_closure: {
  ...
  // Run EngineNews projections until stall
  // EngineNews projections don't use kernel wrapper - they're direct pattern match
  const { result } = run(enginenewsProjections, enginenewsInput, 1000);
...
function detectClosureStructural(projections, input, maxSteps = 100) {
  ...
  // Then run EngineNews to detect closure
  return runEngineNews(traceResult);
}
...
try {
  ...
  console.log('Warning: Could not load enginenews_vectors.json:', e.message);
  enginenewsVectors = { vectors: [] };
}

let enginenewsPassed = 0;
let enginenewsFailed = 0;

for (const vector of enginenewsVectors.vectors) {
  try {
    // Run EngineNews projections directly on the input
    // (vectors already have _detect_closure wrapper with trace)
    const { result } = run(enginenewsProjections, vector.input, 1000);
...
console.log(`\nEngineNews parity tests: ${enginenewsPassed} passed, ${enginenewsFailed} failed`);
const enginenewsAllPassed = enginenewsFailed === 0 && enginenewsPassed > 0;
console.log(`PASS enginenews parity: ${enginenewsAllPassed}`);

// =============================================================================
// Test 11: EngineNews End-to-End (trace + detection)
// =============================================================================
...
try {
  ...
  // Run EngineNews on the trace
  const closureResult = runEngineNews(traceResult);
...
try {
  ...
  // EngineNews correctly detects that 3 recurs
  const closureDetected = closureResult.closure_detected === true;
...
const allPassed = passed && passedStall && pass3a && pass3b && pass3c &&
                  ...
                  enginenewsAllPassed && e2ePassed;
console.log(`All tests passed: ${allPassed}`);
console.log(`\nSecurity hardening (v5 - L3 EngineNews Parity):`);
...
console.log(`\nCore implementation: ~350 lines of JavaScript (with security + EngineNews)`);
console.log(`Projections loaded:`);
console.log(`  - Kernel ops: ${allProjections.length} (kernel + match + subst)`);
console.log(`  - EngineNews: ${enginenewsProjections.length} (closure detection)`);
console.log(`\nThis proves (L3 Substrate Portability - COMPLETE):`);
console.log(`  1. kernel.v1.json runs on JavaScript ✓`);
...
if (process.argv.includes('--json-api')) {
  ...
  try {
    ...
    } else if (request.action === 'run_enginenews') {
      // Run EngineNews closure detection
