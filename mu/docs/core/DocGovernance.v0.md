...
- `mu/docs/core/`, `mu/docs/agents/`, `mu/docs/audit/`, `mu/docs/execution/`
- `mu/docs/cli/`, `mu/docs/schemas/`, `mu/docs/reviews/`
- `tools/agents/` (agent prompt files)

**Exempt Paths** (no governance required):
- `archive/docs/` - Historical, read-only, frozen in time
- `mu/docs/TESTING_PERFORMANCE_ISSUE.md` - Historical context (resolved issue)
...
SUPERSEDED_BY: mu/docs/core/NewDoc.v0.md
...
|---------------------|------|----------|
| A stable definition that rarely changes | REFERENCE | mu/docs/core/ |
| An architectural design that code should follow | DESIGN_SPEC | mu/docs/core/ |
| Documentation that must match current code | IMPLEMENTATION | mu/docs/core/ |
| Notes for a specific agent | REFERENCE | mu/docs/agents/ |
| An audit report | REFERENCE | mu/docs/audit/ |
...
2. **Move to archive:**
   ```bash
   git mv mu/docs/core/OldDoc.v0.md archive/docs/OldDoc.v0.md
   ```
...
| No orphan docs in mu/docs/core/ | `test_doc_governance.py` | CI |
...
3. **Create archive folder** - Move superseded docs to archive/docs/
...
2. All docs in mu/docs/core/ have valid DOC_STATUS headers
