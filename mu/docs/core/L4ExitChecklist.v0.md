...
| `stack_guard` | REDUCIBLE_WITH depth parameter | D009: depth budget as Mu linked-list, mechanism remains host. 145 LOC research analogs (is_mu, match, substitute). Off-by-one boundary parity and failure-mode parity verified. | `pytest tests/research/test_d009_h4_depth_threading.py -q` |
...
**Status:** UNPROVEN (3/4 executable evidence, 2026-03-02). D008 GO (founder-rendered 2026-03-01) authorized D005 production pilot. `eval_step` reclassified from IRREDUCIBLE to REDUCIBLE_WITH (D001 analytical + D002-D003 + D005 executable; see evidence command above). `max_steps` confirmed REDUCIBLE_WITH (D006). `stack_guard` confirmed REDUCIBLE_WITH (D009: depth budget as Mu linked-list; 36 tests including boundary lock and failure-mode parity). `projection_loader` retains REDUCIBLE_WITH classification but lacks executable evidence — prose reasoning alone does not satisfy gate evidence bar. See `mu/docs/core/L4DecisionCard.v0.md` (D001-D009). Next proof target: D010 (projection_loader binary format experiment).
