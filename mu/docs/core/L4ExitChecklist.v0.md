...
**Status:** UNPROVEN (4/4 executable evidence, 2026-03-02). All four primitives now have executable reducibility evidence. D008 GO (founder-rendered 2026-03-01) authorized D005 production pilot. `eval_step` reclassified from IRREDUCIBLE to REDUCIBLE_WITH (D001 analytical + D002-D003 + D005 executable; see evidence command above). `max_steps` confirmed REDUCIBLE_WITH (D006). `stack_guard` confirmed REDUCIBLE_WITH via D009 research analogs (Python only; production stack_guard unchanged; memoization/cycle-detection parity deferred). `projection_loader` confirmed REDUCIBLE_WITH via D010 research artifact (~100 LOC custom recursive TLV decoder, 59 tests including golden byte fixtures and engine-level behavioral parity; research-only, production projection_loader unchanged; I/O, integrity, validation out of scope). See `mu/docs/core/L4DecisionCard.v0.md` (D001-D010). Gate remains UNPROVEN pending formal adjudication — evidence is complete but consensus decision not yet rendered.
...
**RESOLVED (circular dependency):** D001-D003 proved the circular dependency is breakable via staged bootstrap. D005 pilot confirmed in production. **RESOLVED (JSON format):** D010 proved JSON parsing replaceable with ~100 LOC TLV decoder. **ALL FOUR PRIMITIVES** now have executable reducibility evidence (D001-D010). G8 gate awaits formal adjudication.
