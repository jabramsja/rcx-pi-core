...
**Status:** UNPROVEN (3/4 executable evidence, 2026-03-02). D008 GO (founder-rendered 2026-03-01) authorized D005 production pilot. `eval_step` reclassified from IRREDUCIBLE to REDUCIBLE_WITH (D001 analytical + D002-D003 + D005 executable; see evidence command above). `max_steps` confirmed REDUCIBLE_WITH (D006). `stack_guard` confirmed REDUCIBLE_WITH via D009 research analogs (Python only; production stack_guard unchanged; memoization/cycle-detection parity deferred). `projection_loader` retains REDUCIBLE_WITH classification but lacks executable evidence — prose reasoning alone does not satisfy gate evidence bar. See `mu/docs/core/L4DecisionCard.v0.md` (D001-D009). Next proof target: D010 (projection_loader binary format experiment). If depth-threading is ever promoted to production, memoization, cycle-detection, and cross-substrate parity must be addressed in that future wave.
