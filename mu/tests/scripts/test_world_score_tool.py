...
def test_world_score_list_trace(tmp_path: Path):
    ...
    p = _run(["bash", "scripts/world/world_score.sh", str(t), "--json"])
...
def test_world_score_loop_detection(tmp_path: Path):
    ...
    p = _run(["bash", "scripts/world/world_score.sh", str(t), "--json", "--loop"])
    assert p.returncode == 0
    obj = json.loads(p.stdout)
...
def test_world_score_max_steps_violation(tmp_path: Path):
    ...
    p = _run(["bash", "scripts/world/world_score.sh", str(t), "--json", "--max-steps", "3"])
