...
class TestHybridScopeAudit:
    ...
    def test_regenerated_repo_pycache_bytecode_is_not_scope_escape_but_real_source_change_is(
        ...
        # Regression (recovery-scope-drift-exclude-pyc-bytecode): a Step-15 /
        # tier-3 recovery verifies its fix by RUNNING pytest, which regenerates
        # gitignored __pycache__/<mod>.cpython-NNN.pyc bytecode under mu/tests/
        # (and freshly creates the __pycache__ dir on a clean worktree). That
        # regenerated bytecode previously landed in observed_drift, was judged
        # "hybrid observed drift escaped declared scope", and looped the
        # diagnose<->delegate recovery (recovered=False). It must now be excluded
        # from the out-of-scope determination, while a real out-of-scope SOURCE
        # change must STILL fail closed -- proving the exclusion is bytecode-only,
        # not a blanket loosening of scope validation.
        init_hybrid_delegate_tree(tmp_path)
        monkeypatch.setattr(rg_mod, "_capture_hybrid_git_control_tuple", lambda _root: {"stable": True})
        ok, baseline = rg_mod._capture_hybrid_checkpoint(  # ANTICHEAT_OK: baseline before recovery's pytest regenerates bytecode
            tmp_path,
            files_in_scope=["mu/tools/executors/recovery_gate.py"],
            exception_paths=rg_mod._hybrid_exception_paths(),  # ANTICHEAT_OK: test-only helper for exact hybrid exception allowlist
        )
        ...
        # (a) Recovery's pytest run regenerates gitignored bytecode under mu/tests/
        # -- a freshly created __pycache__ dir plus a .cpython-NNN.pyc inside it.
        pycache = tmp_path / "mu" / "tests" / "tools" / "__pycache__"
        pycache.mkdir(parents=True)
        rel_pyc = "mu/tests/tools/__pycache__/test_recovery_gate.cpython-313.pyc"
        (pycache / "test_recovery_gate.cpython-313.pyc").write_bytes(b"\0\0")
        ok, audit = rg_mod._audit_hybrid_checkpoint(  # ANTICHEAT_OK: regenerated mu/tests bytecode must not loop recovery
            tmp_path,
            baseline=baseline,
            files_in_scope=["mu/tools/executors/recovery_gate.py"],
            exception_paths=rg_mod._hybrid_exception_paths(),  # ANTICHEAT_OK: test-only helper for exact hybrid exception allowlist
        )
        # The bytecode (and its fresh cache dir) ARE observed as drift, but are
        # NOT judged an out-of-scope escape, so recovery no longer loops on them.
        assert ok is True
        assert rel_pyc in audit["observed_drift"]
        assert "mu/tests/tools/__pycache__" in audit["observed_drift"]

        # (b) A real out-of-scope SOURCE change (executor_common.py is NOT in the
        # declared scope) STILL fails closed -- even with the bytecode present.
        (tmp_path / "mu" / "tools" / "executors" / "executor_common.py").write_text(
            "after real out-of-scope edit\n",
            encoding="utf-8",
        )
        ok, audit = rg_mod._audit_hybrid_checkpoint(  # ANTICHEAT_OK: real source drift must still be flagged
            tmp_path,
            baseline=baseline,
            files_in_scope=["mu/tools/executors/recovery_gate.py"],
            exception_paths=rg_mod._hybrid_exception_paths(),  # ANTICHEAT_OK: test-only helper for exact hybrid exception allowlist
        ...
        assert "escaped declared scope" in audit["detail"]
        assert "mu/tools/executors/executor_common.py" in audit["detail"]
        # The exclusion held for the bytecode -- it was not what got flagged.
        assert rel_pyc not in audit["detail"]

        # Pin the bytecode-only contract directly: __pycache__ caches/bytecode are
        # exempt; real sources and non-bytecode paths are not.
        assert rg_mod._is_ignored_pycache_bytecode_path(rel_pyc) is True  # ANTICHEAT_OK: bytecode file exempt
        assert rg_mod._is_ignored_pycache_bytecode_path("mu/tests/tools/__pycache__") is True  # ANTICHEAT_OK: cache dir exempt
        assert rg_mod._is_ignored_pycache_bytecode_path("mu/tools/executors/executor_common.py") is False  # ANTICHEAT_OK: real source not exempt
        assert rg_mod._is_ignored_pycache_bytecode_path("mu/tests/tools/__pycache__/notes.txt") is False  # ANTICHEAT_OK: non-bytecode in cache not exempt

    def test_repo_pycache_exemption_rejects_symlinked_cache_dir_escaping_repo(
        self, tmp_path, monkeypatch
    ):
        # Finding 2 (bridge round 1): the generalized pycache exemption matches the
        # path STRING, so a mu/tests/.../__pycache__ that is actually a SYMLINK
        # escaping the worktree must NOT be exempted -- it must stay observed drift
        # and fail closed, exactly like the .scratch/__pycache__ precedent. Real
        # regenerated caches stay exempt (covered above); only the symlink-escape
        # variant is pinned here, so the exemption can never become a scope hole.
        init_hybrid_delegate_tree(tmp_path)
        monkeypatch.setattr(rg_mod, "_capture_hybrid_git_control_tuple", lambda _root: {"stable": True})
        ok, baseline = rg_mod._capture_hybrid_checkpoint(  # ANTICHEAT_OK: baseline before symlink escape planted
            tmp_path,
            files_in_scope=["mu/tools/executors/recovery_gate.py"],
            exception_paths=rg_mod._hybrid_exception_paths(),  # ANTICHEAT_OK: test-only helper for exact hybrid exception allowlist
        )
        ...
        outside = tmp_path.parent / "outside_repo_pycache_dir"
        outside.mkdir(exist_ok=True)
        pycache_link = tmp_path / "mu" / "tests" / "tools" / "__pycache__"
        # No skip guard (this wave's packet forbids skip/xfail): the symlink-escape
        # fail-closed proof must always run. POSIX symlinks are available on the
        # darwin/Linux test hosts; an unsupported platform fails loudly rather than
        # silently masking this security-relevant assertion.
        pycache_link.symlink_to(outside, target_is_directory=True)

        ok, audit = rg_mod._audit_hybrid_checkpoint(  # ANTICHEAT_OK: __pycache__ symlink escaping repo must fail closed
            tmp_path,
            baseline=baseline,
            files_in_scope=["mu/tools/executors/recovery_gate.py"],
            exception_paths=rg_mod._hybrid_exception_paths(),  # ANTICHEAT_OK: test-only helper for exact hybrid exception allowlist
        ...
        assert ok is False
        assert "escaped declared scope" in audit["detail"]
        assert "mu/tests/tools/__pycache__" in audit["detail"]
        # The string shape still matches, but on-disk validation rejects the symlink.
        assert rg_mod._is_ignored_pycache_bytecode_path("mu/tests/tools/__pycache__") is True  # ANTICHEAT_OK: shape still matches
        assert rg_mod._validate_ignored_pycache_bytecode_path(tmp_path, "mu/tests/tools/__pycache__") is False  # ANTICHEAT_OK: symlink cache dir not exempted

    def test_repo_pycache_exemption_rejects_symlinked_pyc_file_escaping_repo(
        self, tmp_path, monkeypatch
    ):
        # Finding 2 (bridge round 1): a real __pycache__ dir containing a .pyc that
        # is a SYMLINK escaping the worktree must fail closed on the .pyc -- the real
        # cache dir stays exempt, the symlinked bytecode does not. Proves the on-disk
        # validation is per-path and not fooled by a bytecode-shaped link target.
        init_hybrid_delegate_tree(tmp_path)
        monkeypatch.setattr(rg_mod, "_capture_hybrid_git_control_tuple", lambda _root: {"stable": True})
        ok, baseline = rg_mod._capture_hybrid_checkpoint(  # ANTICHEAT_OK: baseline before symlinked .pyc planted
            tmp_path,
            files_in_scope=["mu/tools/executors/recovery_gate.py"],
            exception_paths=rg_mod._hybrid_exception_paths(),  # ANTICHEAT_OK: test-only helper for exact hybrid exception allowlist
        )
        ...
        outside = tmp_path.parent / "outside_repo_artifact.pyc"
        outside.write_bytes(b"\0\0")
        pycache = tmp_path / "mu" / "tests" / "tools" / "__pycache__"
        pycache.mkdir(parents=True)
        rel_pyc = "mu/tests/tools/__pycache__/test_recovery_gate.cpython-313.pyc"
        # No skip guard (this wave's packet forbids skip/xfail): the symlinked-.pyc
        # fail-closed proof must always run. POSIX symlinks are available on the
        # darwin/Linux test hosts; an unsupported platform fails loudly rather than
        # silently masking this security-relevant assertion.
        (pycache / "test_recovery_gate.cpython-313.pyc").symlink_to(outside)

        ok, audit = rg_mod._audit_hybrid_checkpoint(  # ANTICHEAT_OK: symlinked .pyc escaping repo must fail closed
            tmp_path,
            baseline=baseline,
            files_in_scope=["mu/tools/executors/recovery_gate.py"],
            exception_paths=rg_mod._hybrid_exception_paths(),  # ANTICHEAT_OK: test-only helper for exact hybrid exception allowlist
        ...
        assert ok is False
        assert "escaped declared scope" in audit["detail"]
        assert rel_pyc in audit["detail"]
        # Real cache dir stays exempt; only the symlinked bytecode fails closed.
        assert rg_mod._validate_ignored_pycache_bytecode_path(tmp_path, "mu/tests/tools/__pycache__") is True  # ANTICHEAT_OK: real cache dir exempt
        assert rg_mod._validate_ignored_pycache_bytecode_path(tmp_path, rel_pyc) is False  # ANTICHEAT_OK: symlinked .pyc not exempted
        # A bytecode-shaped path with no on-disk object is benign churn (precedent parity).
        assert rg_mod._validate_ignored_pycache_bytecode_path(  # ANTICHEAT_OK: missing bytecode is benign
            tmp_path, "mu/tests/tools/__pycache__/ghost.cpython-313.pyc"
        ) is True
...
@pytest.fixture(params=[False, True], ids=["bytecode-enabled", "bytecode-disabled"])
def post_reentry_authority(tmp_path, monkeypatch, request):
    ...
    def recover():
        ...
        # Exercise ordinary Python imports even when the enclosing recovery
        # pytest gate disables or redirects bytecode writes.
        with monkeypatch.context() as import_settings:
            import_settings.setattr(sys, "dont_write_bytecode", request.param)
            import_settings.setattr(sys, "pycache_prefix", None)
            recovery = rg_mod.attempt_recovery(repo, result, spec.wave_id, bus_dir=bus)
            assert sys.dont_write_bytecode is request.param
...
def test_post_reentry_required_authority_reaches_existing_consumer(post_reentry_authority, mutate_candidate):
    ...
    assert recovery["recovered"] is True, recovery
    assert not list((f.repo / "mu" / "tools" / "executors").rglob("__pycache__"))
...
def test_post_reentry_verifies_rebuilt_receipt_before_success(post_reentry_authority, monkeypatch, damage):
    ...
    recovery, result = f.recover()
    assert len(prepared) == 1, recovery
    assert recovery["recovered"] is False, recovery
