...
class TestWaveIdDerivation:
    ...
    def test_range_mode_uses_parent_wave_when_packet_is_on_same_wave_append(
        self, tmp_path: Path
    ) -> None:
        repo = self._init_repo(tmp_path)
        packet = repo / "reports" / "control_plane" / "child-wave.md"
        ...
        (repo / "TASKS.md").write_text(
            ...
            "bootstrap_endgame_policy: SUBSTRATE_INDEPENDENT_MINIMAL_BOOTSTRAP. "
            "boot0_track_id: V1. boot0_progress_state: HOLD.\n"
            "- PR CI repair append (2026-04-21T00:00:00Z, child-wave): "
            "Class: L4_ENABLER. Packet: `reports/control_plane/child-wave.md`.\n",
            encoding="utf-8",
        )
        subprocess.run(
            ["git", "add", "TASKS.md", str(packet), str(runtime_file)],
            cwd=repo,
            check=True,
        )
        subprocess.run(["git", "commit", "-m", "range"], cwd=repo, check=True, capture_output=True)
