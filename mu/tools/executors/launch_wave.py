    ...
    *,
    ...
    if config.candidate_authority_enabled():
        ...
        authority_metadata = {
            ...
            "spec_identity": _ca.authority_spec_identity(
                Path(repo_root),
                spec,
                authority_required=bool(config.pre_review_authority),
            ),
        }
        target_branch_authority = _launch_target_branch_authority(repo_root, config)
        if target_branch_authority is not None:
...
def _launch_wave_bound_target_branch(
    observed_branch: str,
    *,
    wave_id: str,
    branch_prefix: str,
...
def _launch_target_branch_authority(
    repo_root: Path,
    config: WaveConfig,
) -> dict[str, str] | None:
    """Persist the launch-owned fresh/restart branch when the lane is opted in."""
    ...
    if result.returncode != 0:
        return None
    observed_branch = result.stdout.strip()
    if not observed_branch or observed_branch in {"dev", "main", "master", "HEAD"}:
        return None
    if "/" not in observed_branch:
        return None
    branch_prefix = observed_branch.split("/", 1)[0].strip()
    target_branch = _launch_wave_bound_target_branch(
        observed_branch,
        wave_id=config.wave_id,
...
def _prestage_l4_indicator(repo_root: Path, config: WaveConfig) -> None:
    ...
    the L4 ``--staged`` indicator artifact as ABSENT, because the indicator is
    generated by ``commit_executor`` Step 5 (``collect_and_stage_indicator``), which
    runs LATER -- at commit time -- so it is always missing at Phase-B-review time.
    The reviewer then marks that acknowledged-EXPECTED finding disposition=blocking
    and the wave strands, requiring a hand-finish.

    When the WaveConfig declares BOTH a non-empty ``indicator_collection_command``
    and a non-empty ``indicator_artifact_ref``, this runs the collection command
    (cwd=``repo_root``) and ``git add``s the artifact path, so the L4 ``--staged``
    contract finds the indicator PRESENT at Phase-B review. The collection command
    and ``git add -f`` mirror Step 5's own pattern, so the pre-staged artifact is
    byte-equivalent to the one Step 5 would produce.

    STRICTLY BEST-EFFORT / FAIL-OPEN. Any failure -- an un-parseable or un-runnable
    command, a non-zero collector exit, a missing artifact, or a ``git add``
    failure -- logs a WARNING and RETURNS WITHOUT RAISING. It MUST NEVER break or
    abort a wave launch: ``commit_executor`` Step 5 remains the idempotent authority
    that regenerates and stages the indicator at commit, so a failed (or skipped)
    pre-stage is harmless. This function changes NO gate semantics, the L4
    consistency-check, or Step 5; it only pre-generates the artifact at launch.

    No-op (silent, no warning) when either indicator field is empty -- a wave that
    declares no indicator.
    """
    command = (config.indicator_collection_command or "").strip()
    artifact_ref = (config.indicator_artifact_ref or "").strip()
    if not command or not artifact_ref:
        return  # no indicator declared -> nothing to pre-stage (silent no-op)

    def _warn(detail: str) -> None:
        # Mirror the module's existing stderr style (see main()); fail-open, so a
        # warning -- never an exception -- is the loudest this gets.
        print(
            f"launch_wave: WARNING: L4 indicator pre-stage skipped ({detail}); "
            "commit_executor Step 5 remains the authority that regenerates and "
            "stages the indicator at commit.",
            file=sys.stderr,
        )
    ...
    try:
        ...
        _warn(f"indicator_collection_command is not a parseable command: {exc}")
        return
    if not argv:
        _warn("indicator_collection_command is empty after parsing")
        return
    ...
    try:
        ...
        _warn(f"indicator_collection_command could not run: {exc}")
        return
    if result.returncode != 0:
        _warn(
            f"indicator_collection_command exited {result.returncode}: "
            f"{(result.stderr or '').strip()[:300]}"
        )
        return

    if not (repo_root / artifact_ref).exists():
        _warn(f"indicator artifact {artifact_ref!r} absent after collection")
        return

    try:
        add_result = subprocess.run(
            ["git", "add", "-f", "--", artifact_ref],
            cwd=str(repo_root),
            text=True,
            capture_output=True,
            check=False,
        ...
        _warn(f"git add of indicator {artifact_ref!r} could not run: {exc}")
        return
    if add_result.returncode != 0:
        _warn(
            f"git add of indicator {artifact_ref!r} exited "
            f"{add_result.returncode}: {(add_result.stderr or '').strip()[:300]}"
        )
...
def run_wave_setup(
    ...
    *,
    ...
    if candidate_authority_spec is not None:
        try:
            spec = _ca.load_authority_spec(candidate_authority_spec)
            _ca.guard_candidate_scope_before_mutation(repo_root, spec)
        except _ca.CandidateAuthorityError as exc:
            raise LaunchWaveError(
                "candidate authority scope guard failed before L4 indicator "
                f"pre-stage: {exc}"
            ) from exc

    # Best-effort, fail-open: pre-stage the L4 ``--staged`` indicator artifact so the
    # Phase-B review finds it PRESENT. The indicator is otherwise generated by
    # commit_executor Step 5 (which runs later, at commit), so it is always ABSENT
