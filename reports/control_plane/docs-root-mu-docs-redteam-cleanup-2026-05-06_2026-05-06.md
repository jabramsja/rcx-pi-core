...
- Non-authorizing routing diagnostic: the current
  `.agent_bus/meta/post_merge_routing.json` is stale and must not be used as
  proof of route authority. `.agent_bus/meta/post_merge_routing.json:2` through
  `.agent_bus/meta/post_merge_routing.json:4` still name `ROUTE_PHASE_A`, this
  wave, and `[NEXT-CODEX-POST-REDTEAM]`, and
  `.agent_bus/meta/post_merge_routing.json:7` through
  `.agent_bus/meta/post_merge_routing.json:14` carry only a candidate/bounded
...
| --- | --- | --- | --- | --- | --- | --- |
| F1 | DOC_ACCURACY | Non-blocking | `mu/docs` governance/indexes | `mu/docs/README.md:20` through `mu/docs/README.md:21` says the file is generated by `mu/tools/docs/generate_docs_index.py`; `mu/tools/docs/generate_docs_index.py:132` through `mu/tools/docs/generate_docs_index.py:140` indexes every top-level `*.md`; `python3 mu/tools/docs/generate_docs_index.py --check` exited `1` with `mu/docs/README.md is OUT OF DATE`; `comm -23` inventory commands showed missing active index rows for `mu/docs/agents/AgentBridgeProtocol.v0.md`, `mu/docs/agents/PipelineRecovery.v0.md`, `mu/docs/core/L3SubstrateArchitecture.v0.md`, `mu/docs/core/MuDagAbiSpike.v0.md`, `mu/docs/core/NorthStarSemantics.v0.md`, `mu/docs/core/OntologyPromotionContract.v0.md`, and `mu/docs/core/TypedNumericEnvelopes.v0.md`. | Regenerate/update `mu/docs/README.md`; update its `LAST_VERIFIED` header to 2026-05-06 if the generated content is accepted. | Yes |
...
1. `mu/docs/README.md`
   - Edit: regenerate/update the active index so it includes every active
     non-archive markdown file listed in this packet.
...
- This packet, the same-wave `TASKS.md` tracker sync note, and the three
  explicitly authorized Phase B docs cleanup files are the only files edited by
  the Bridge Round 2 plus hook closeout remediation.
...
- Phase B is authorized only for the three specific docs cleanup edits listed
  in this packet.
- No archive move is authorized.
