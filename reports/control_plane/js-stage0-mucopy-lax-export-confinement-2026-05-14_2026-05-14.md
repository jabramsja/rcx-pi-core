...
Purpose: Plan the selected N3 successor slice: a bounded JS Stage0 public export confinement that prevents externally reachable `muCopy` from entering lax `rejectNonMu=false` mode, while preserving private/internal lax copying needed by VM-owned bundle/capture template materialization.

## Scope

This packet rewrite scope:
...
Locked Phase B implementation write set, if this plan is later accepted:
...
- This turn is a packet rewrite only; it does not authorize implementation edits.
...
- Pending work is exactly one bounded `/mu` structural host-surface reduction candidate: JS Stage0 exported `muCopy` lax-mode confinement.
...
- Focused validation for implementation must include:
