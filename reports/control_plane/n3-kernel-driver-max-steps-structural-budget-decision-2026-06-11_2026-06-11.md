...
5. Did not fire. Round-1 PR bot review flagged that pairing the `Phase-A-Lock: LOCKED` header with this condition's "flips to LOCKED mid-wave" wording reads as a self-contradiction if left unexplained, so the resolution is recorded explicitly instead of being read past:
   - Lock-lifecycle code truth: the canonical control line is created `UNLOCKED` by the Phase A packet template and transitions `UNLOCKED -> LOCKED` exactly once, applied mechanically by `mu/tools/executors/phase_a_executor.py:lock_plan` once the plan's bridge review converges (the executor's "Set Phase-A-Lock: LOCKED" control-flow step; idempotent, fail-closed on missing, malformed, or duplicated control lines). Phase B fails closed on an unlocked plan (`mu/tools/executors/phase_b_executor.py:validate_inputs`), so every Decision Record this pipeline can produce -- including this one, and including any STOP record for this condition, which would itself be written under `LOCKED` -- is necessarily written after that designed flip. Control-plane packets use the same phrase as the implementation GO signal, not a stop trigger (e.g. `reports/control_plane/control_packet_line_ref_lint_2026-06-01.md`: "implementation must not begin before Phase-A-Lock flips to LOCKED").
   - Trigger semantics: reading the designed A->B handoff itself as the trigger would make condition 5 fire vacuously on every wave the pipeline executes, so it cannot be the guarded event. The coherent trigger -- parallel to the tracker-note half of the same sentence -- is an unexpected lock-state change under the in-flight wave at execution time (authorization/premise drift after dispatch), e.g. the line being flipped by anything other than the executor's own convergence step while the decision work is in flight.
   - That guarded event did not occur: Phase B entry validation passed with the single canonical `LOCKED` header line (the packet's other `Phase-A-Lock` mentions in Purpose/Constraints/Stop-conditions are quoted policy text, not control lines, and `lock_plan` inspects the header only); the lock did not change during execution of WI-1..WI-5; and the Phase-A-only premise the guard protects held throughout -- zero runtime edits, zero marker movement, zero-delta ratchet evidence (WI-5).
   - Tracker-note half: the TASKS.md tracker note for this wave id was verified present and unchanged at execution time; its sha256 binding is recorded in the Commit Path Truth Refresh block of this packet.
