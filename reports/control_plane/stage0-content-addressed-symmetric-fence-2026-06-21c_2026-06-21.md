...
6. Set the L4_STRUCTURAL tracker-note fields the launcher does not pre-fill: workload_target=host_debt_reduction and host_semantics_delta (the in-match scalar type-dispatch reduction: 4 scalar isinstance branches -> 1 content-addressed mu_hash equality + 1 input-side list fail-close; the `@host_*` marker ratchet remains unchanged, net_host_semantic_delta=0 -- not a marker-count decrease); the staged L4 contract requires both.
7. Run evidence_command + post_gate_contract_sweep; confirm host-semantics ratchet remains unchanged (net_host_semantic_delta=0; no marker increase), authority inventory unchanged, node eval_step clean, parity gates green; emit the indicator.
...
- Host-semantics ratchet must NOT increase (net_host_semantic_delta=0 is the expected non-regressing outcome -- the scalar type-dispatch collapses in-function while `_stage0_match` retains its single `@host_builtin` marker); keep None/null + compound(dict) branch + worklist + depth guard + non-linear conflict UNCHANGED; bounded to the named files.
...
- Stop done when evidence_command + post_gate_contract_sweep pass, the host-semantics ratchet does NOT regress (net_host_semantic_delta=0; no `@host_*` marker increase), and the indicator is collected.
...
- host-semantics ratchet remains unchanged (net_host_semantic_delta=0; no marker increase); authority inventory unchanged; node eval_step clean; parity + crash gates green.
