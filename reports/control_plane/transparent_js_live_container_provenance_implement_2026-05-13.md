...
- Broad pre-push audit after local commit `b36f18d3` failed with `8 failed, 6890 passed, 18 skipped in 348.19s`. Root-cause evidence: raw JS compound fixtures in the broad suite now hit the strengthened boundary at `normalize.js:165` (`normalize: compound value lacks trusted Mu provenance`), `bootstrap_core.js:297` (`Invalid Mu input to step()`), and `bootstrap_core.js:99` (`Invalid Mu pattern in match()`); the source-shape guard also needed to acknowledge the intentional engine `container_factory` dependency introduced by producer provenance. The repair keeps runtime behavior unchanged, converts only affected JS test fixtures to existing Stage0 `muCopy` Mu-origin values, and updates the engine dependency inventory. Focused evidence: the exact 8-failure repro command exits 0 with `8 passed in 0.68s`; affected file suite `tests/engine/test_normalization_roundtrip.py tests/structural/test_engine_pipeline_discipline.py tests/l4_gates/test_terminal_classification_parity_gate.py tests/l4_gates/test_redteam_hardening_gate.py` exits 0 with `211 passed in 22.69s`.
