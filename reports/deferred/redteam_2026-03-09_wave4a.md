...
- **Status:** RESOLVED (2026-03-13). Added `_reconcile_parity()` parallel-walk + `_substitute_direct()` in `subst_mu.py`. Root head/tail bodies short-circuit to `_substitute_direct`; non-head/tail roots go through `denormalize_from_match` then `_reconcile_parity` to restore raw binding values at var sites and preserve nested head/tail structures. 10 new parity tests in `TestSubstParityNestedHeadTail` (test_subst_parity.py). Ratchet baseline updated 267→269 total, 190→192 authority.
