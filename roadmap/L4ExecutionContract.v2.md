...
18. **Workload target proof binding (L4_STRUCTURAL):** When a `workload_target` has registered evidence files in `WORKLOAD_TARGET_EVIDENCE`, the enforcer checks: (a) contract test files exist on disk (hard fail if missing), (b) at least one evidence file appears in the wave's changed files or is referenced in gate scripts (`tools/audit_fast.sh`, `tools/audit_all.sh`, `scripts/green_gate.sh`), (c) `evidence_command` references at least one evidence test module name. Targets with empty evidence lists are exempt (proof binding not yet established).
