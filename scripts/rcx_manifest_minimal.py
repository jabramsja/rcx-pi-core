...
def main() -> None:
    ...
    if not pack.exists():
        raise SystemExit(
            "Missing rcx_pack_minimal.txt. Run: scripts/rcx_packlist.py --mode minimal"
        )
    ...
    buckets = [
        Bucket(
            "Top-level",
            "Project entrypoints and task spine.",
            lambda p: p in ("README.md", "TASKS.md"),
        ),
        Bucket(
            "CI / Gates",
            "Workflows + deterministic gate scripts.",
            lambda p: p.startswith(".github/workflows/")
            or p.startswith("scripts/check_")
            or p in ("scripts/green_gate.sh",),
        ),
        Bucket(
            "Schemas",
        ...
        Bucket(
            "Scripts",
            "Helper scripts for PR watching, sync, rehydrate, etc.",
            lambda p: p.startswith("scripts/") and not p.startswith("scripts/check_"),
        ),
        Bucket(
            "Tests",
            "Smoke/contract tests that define behavior.",
            lambda p: p.startswith("scripts/tests/") or p.startswith("tests/"),
        ),
    ]
