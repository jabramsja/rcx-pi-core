...
def build_pack(
    files: list[str], globs: list[str], hard_exclude_prefixes: list[str]
) -> list[str]:
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Build deterministic re-upload packlists from tracked repo files."
    )
    ap.add_argument("--mode", choices=["minimal", "extended", "both"], default="both")
    ap.add_argument(
        "--include-fixtures",
        action="store_true",
        help="Include docs/fixtures in extended packs.",
    )
    ap.add_argument("--no-tar", action="store_true", help="Do not create tarballs.")
    ap.add_argument(
        "--outdir", default=".", help="Output directory (relative to repo root)."
    )
    args = ap.parse_args()
    ...
    extended_globs = (
        EXTENDED_GLOBS_WITH_FIXTURES if args.include_fixtures else EXTENDED_GLOBS_BASE
    )
    ...
    for pack in packs:
        ...
        print(f"\n== pack: {pack.name} ==")
        print(
            f"listed: {len(raw)}   kept(existing): {len(kept)}   missing(filtered): {len(missing)}"
        )
        if missing:
        ...
        if pack.out_tar:
            ...
            subprocess.run(
                ["tar", "-czf", str(out_tar), "-T", str(out_list)], cwd=root, check=True
            )
