...
class TestPreCommitDocDetection:
    ...
    def test_tools_agents_detected(self):
        """tools/agents/ must still trigger doc tests."""
        pattern = _extract_docs_pattern()
        assert re.search(pattern, "tools/agents/verifier_prompt.md"), (
            f"Pattern {pattern!r} does not match tools/agents/ paths"
        )
